...
const char * const BITCOIN_CONF_FILENAME = "gem.conf";
const char * const BITCOIN_PID_FILENAME = "gemd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "gem" is a composite category enabling all Gem-related debug output
                if(ptrCategory->count(std::string("gem"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GemCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GemCore
    // Mac: ~/Library/Application Support/GemCore
    // Unix: ~/.gemcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GemCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GemCore";
#else
    // Unix
    return pathRet / ".gemcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty gem.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override gem.conf
