...
// which are part of the Gem repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="gem-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /gem-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "gempay-gem/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from gempay-gem/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'gempay-gem/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from gempay-gem/develop"
            }
          }

          if (hasCache) {
            sh "cd /gem-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /gem-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /gem-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /gem-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /gem-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /gem-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /gem-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
