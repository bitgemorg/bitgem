...
# gem_hash
RUN git clone https://github.com/gempay/gem_hash
RUN cd gem_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} gem
RUN useradd -u ${USER_ID} -g gem -s /bin/bash -m -d /gem gem
...
RUN mkdir /gem-src && \
  ...
  chown $USER_ID:$GROUP_ID /gem-src && \
